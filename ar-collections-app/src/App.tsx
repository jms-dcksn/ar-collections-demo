...
export default function App() {
  ...
  const content = route.startsWith('/disputes/') ? <DisputeDetailPage instanceId={route.split('/').at(-1)} /> : route === '/analytics' ? <PlaceholderPage title="Collections analytics" /> : route === '/activity' ? <PlaceholderPage title="Recent activity" /> : route === '/settings' ? <PlaceholderPage title="Workspace settings" /> : <DashboardPage />
